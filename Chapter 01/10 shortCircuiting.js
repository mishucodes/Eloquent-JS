//I think the AND & OR gates work very differently here. They don't return booleans, but actual values (their operands).
...
//MORE IMPORTANTLY, THE AUTHOR WRITES:
...
//TLDR:
//In JavaScript, && and || return one of their operands, not just booleans.
//|| returns the first truthy value (or the last if none), useful for fallbacks.
//&& returns the first falsy value (or the last if none), useful for guard checks.
//?? is like || but only treats null/undefined as empty, not 0, "" or NaN.
//Both && and || short-circuit, so the right side is only evaluated if needed.
//Unlike C (where they always return 0/1), JS’s design makes them handy for defaults and conditional execution.